...
const CourseSettings = () => {
  // 1. สร้าง State สำหรับเก็บข้อมูลวิชาจาก API
  const [courses, setCourses] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");

  // 2. ดึงข้อมูลจาก Backend เมื่อ Load หน้าเว็บ
  useEffect(() => {
  ...
  // ฟังก์ชันสำหรับกรองข้อมูล (Search)
          ...
          <div className="relative z-10 flex justify-between items-start">
            <div>
              ...
              <p className="text-blue-100 opacity-90 pl-1">
                Manage your {courses.length} courses and attendance rules.
              </p>
            </div>
          </div>
        ...
        <div className="px-10 -mt-20 pb-10 relative z-20">
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 min-h-[600px] flex flex-col">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
              <h2 className="text-xl font-bold text-gray-800">All Courses Configuration</h2>
              <div className="flex gap-3 w-full md:w-auto">
                <div className="relative group flex-1 md:flex-none">
                  ...
                  <input 
                    ...
                    className="w-full md:w-64 pl-10 pr-4 py-2.5 bg-gray-50 border-transparent rounded-xl focus:bg-white focus:border-blue-500 transition-all text-sm font-medium"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto flex-1">
              {loading ? (
                <div className="flex justify-center items-center h-64 text-gray-400 font-medium">Loading courses...</div>
              ) : (
                <table className="w-full min-w-[900px]">
                  ...
                  <tbody className="text-sm">
                    {filteredCourses.map((course) => (
                      <tr key={course.id} className="border-b border-gray-50 hover:bg-gray-50/80 transition h-20">
                        <td className="pl-4">
                          <div className="flex flex-col">
                            <span className="font-bold text-gray-800 text-base">{course.name}</span>
