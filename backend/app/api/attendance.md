...
@router.get("/attendance/report/{session_id}")
async def get_attendance_report(session_id: int, db: AsyncSession = Depends(get_db)):
    # 1. ดึงข้อมูล Session และหาว่าวิชานี้คือวิชาอะไร (course_id)
    session_result = await db.execute(
    ...
    # 2. ดึงรายชื่อนักเรียน "ทุกคน" ที่ลงทะเบียนเรียนวิชานี้ (Enrollment)
    enroll_result = await db.execute(
        select(Enrollment)
        .options(selectinload(Enrollment.student))
        .where(Enrollment.course_id == session.course_id)
    )
    enrolled_students = [e.student for e in enroll_result.scalars().all()]

    # 3. ดึงประวัติการสแกนหน้า (Attendance) เฉพาะของคาบเรียนนี้
    att_result = await db.execute(
        select(Attendance).where(Attendance.session_id == session_id)
    )
    attendances = {
        att.student_id: att for att in att_result.scalars().all()
    }  # แปลงเป็น Dict เพื่อให้ค้นหาง่าย

    # 4. ประกอบร่างข้อมูลส่งให้ Frontend
    report_data = []
    for student in enrolled_students:
        record = attendances.get(student.id)  # เช็คว่าเด็กคนนี้มี record การสแกนไหม

        # สมมติว่าเอารหัส นศ. จากอีเมล (ถ้าใน User table ไม่มีช่อง student_id ตรงๆ)
        student_id_str = student.email.split("@")[0] if student.email else "Unknown"

        report_data.append(
            {
                "id": student.id,
                "name": student.full_name or "No Name",
                ...
                else (0.5 if record and record.status == "late" else 0),
            }
        )
